<template>
  <div class="cpn-result">
    <div class="content" v-if="currentStatus.resultStatus == 'failed'">
      <div class="img">
        <img :src="fail_IMG" alt="" />
      </div>
      <div class="tips">
        <p>您的{{currentStatus.title}}审核于 {{ currentStatus.time && timestampToTime(currentStatus.time) }} 未通过</p>
        <p>建议查看审批详情修改入驻信息再提交审核</p>
      </div>
      <div class="errinfo">审批意见：{{ currentStatus.info || '-' }}</div>
    </div>
    <div class="content" v-else-if="currentStatus.resultStatus == 'succeeded'">
      <div class="img">
        <img :src="success_IMG" alt="" />
      </div>
      <div class="tips">
        <p>
          您的{{currentStatus.title}}审核于 {{ currentStatus.time && timestampToTime(currentStatus.time) }} 顺利通过
        </p>
    ...
    <div class="content" v-else-if="currentStatus.resultStatus == 'netin'">
      <div class="img">
        <img :src="success_IMG" alt="" />
      </div>
      <div class="tips">
    ...
    <div class="btn">
      <el-button type="primary" v-if="currentStatus.resultStatus == 'failed'" @click="reset"
        >重新填写</el-button
      >
      <el-button type="primary" v-if="currentStatus.resultStatus == 'succeeded'" @click="next"
        >下一步</el-button
      >
      <el-button
        type="primary"
