  ...
  <div>
    <el-card v-if="loginType === 'distributor'">
      <el-row style='marginBottom: 0px'>
        <el-form :model="topforms" ref="topforms" :rules='topformsrules'>
          <el-row type='flex' justify='space-around' style='marginBottom: 0px'>
            <el-col :span="5" class="left-cols-css">
              ...
              <el-form-item prop="withdrawal_amount" v-if="distributormoneyList.withdraw_method === 2" >
                提现 <el-input v-model="topforms.withdrawal_amount" size="mini" :precision="2" @input="oninput" style="width:150px" /> 元
              </el-form-item>
              <p style="margin-bottom:15px" v-if="distributormoneyList.withdraw_method === 2">提现限额：满 <span style="color:red">¥ {{distributormoneyList.distributor_money}}<span style="color:#000" > 元，可提现</span></span></p>
              <el-button type="primary" v-if="distributormoneyList.withdraw_method === 2" @click="onSumbit">提现</el-button>
            </el-col>
            <el-col :span="16" class="right-cols-css">
              <p>1、当前可提现金额需大于或等于提现限额才可进行提现；</p>
              <p>2、提现操作请在10:00:00-23:59:59进行(仅可提现金额大于或等于提现限额)；</p>
              <p>3、根据支付公司逻辑，申请提现后一个工作日到账，节假日顺延具体提现信息请查询提现记录。</p>
            </el-col>
          </el-row>
        </el-form>
      </el-row>
        ...
        <el-row style='marginBottom: 0px'>
          ...
          <el-col :span="6">
            <el-form-item label="提现订单号">
              <el-input v-model="formData.order_id" style="width: 100%;" size='small' placeholder="请输入提现订单号"></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="12" v-if="loginType !== 'distributor'">
          ...
          <el-col :span="4" :push='8' v-if="loginType !== 'distributor'" style="margin-left:10px">
            <div class="flex-right">
              <el-button size='small' @click="handleReset">重置</el-button>
              <el-button size='small' type="primary" @click="getCompanyDataList()">搜索</el-button>
            </div>
          </el-col>
        </el-row>
        <el-row :gutter="20" v-if="loginType === 'distributor'">
    ...
    <el-card>
      <el-row style='marginBottom: 0px'>
        <el-col :span="4" class="total-num-box">
          <p class="total-num-top-fonts">{{allAccountList.count}}</p>
        ...
        <el-col :span="4" class="total-num-box">
          <p class="total-num-top-fonts">¥ {{(allAccountList.finish_total_amt / 100).toFixed(2)}}</p>
        ...
        <el-col :span="4" class="total-num-box">
          <p class="total-num-top-fonts">¥ {{(allAccountList.fail_total_amt / 100).toFixed(2)}}</p>
    ...
    <el-card>
      ...
      <el-table :data="allListData" stripe border style="width: 100%" v-loading="loading">
        ...
        <el-table-column prop="order_id" label="提现订单号" width="180"></el-table-column>
        <el-table-column prop="bind_card_id" label="到账银行卡号" width="180"></el-table-column>
        <el-table-column label="提现金额">
          <template slot-scope="scope">￥{{(scope.row.trans_amt/100).toFixed(2)}}</template>
        </el-table-column>
        <el-table-column prop="distributor_name" label="店铺名称" v-if="loginType !== 'distributor'" width="180"></el-table-column>
        <el-table-column prop="login_name" label="操作人" width="160"></el-table-column>
        ...
        <el-table-column prop="resp_desc" label="备注" width='400'>
          <template slot-scope="scope">
            <p style="color:#027DB4">{{scope.row.resp_desc}}</p>
          </template>
        </el-table-column>
      </el-table>
    ...
    methods: {
      ...
      onSumbit () { // 设置提现金额
        ...
        this.$refs.topforms.validate((valid) => {
          if (valid) {
            this.$confirm(`<div style='margin-left:45px'>
      ...
      getCompanyDataList () {
        ...
        withdrawgetList({...this.formData, ...this.params }).then(res => { // 列表和统计
        ...
        getStoreAccount({...this.formData, ...this.params }).then((res) => { // 获取当前可提现金额
          this.withdrawal_balance = res.data.data.totle.withdrawal_balance;
        })
        getWithdrawset().then(res => { // 获取提现限额和提现方式
