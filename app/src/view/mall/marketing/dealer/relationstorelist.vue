<template>
  <div class="cus-dealer-rel-page">
    <div>
      ...
      <AddShipModal :visible='visibleModal' :info='detailData' :content='modalContent' @handleClick='handleModalClick' />
    </div>
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    ...
    handleModalClick (visible, type, row) {
      if (row) {
        this.detailData = {...row, store_name: row.name, username: this.username, operator_id: this.dealer_id}
        this.modalContent = row.audit_state == '3' ? '如新增关联，已入网成功的店铺需重新设置店铺所占分帐比例。' : `请确认是否将【${row.name}】与【${this.username}】关联`
      }
