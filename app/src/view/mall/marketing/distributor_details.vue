<template>
  <div class="cus-details">
      ...
      <el-tabs v-model="activeName" class="cus-details-tabs">
        ...
        <el-tab-pane label="开户信息" name="second" v-if="is_openAccount">
          ...
          <BaseModal
            ...
            :labelList="is_rel_dealer && is_openAccount && member_type === 'corp' ? enterSplitAccountList : splitAccountList"
        ...
        <el-tab-pane v-if="!is_openAccount && loginType === 'distributor'" label="开户信息" name="second">
    ...
    <el-row v-if="is_rel_dealer && loginType === 'normal'" class="cus-details-bot">
      <el-button type='primary' @click="handleModalClick(true, is_openAccount)" size='small' plain>解除关联</el-button>
    </el-row>
...
export default {
  ...
  methods: {
    ...
    getTabDetail () {
      ...
      .then((response) => {
        const { is_openAccount, is_rel_dealer, adapayMemberInfo, dealer } = response.data.data || {}
        const { basicInfo: { auto_sync_goods, is_ziti, is_delivery, company_dada_open } } = adapayMemberInfo || {}
