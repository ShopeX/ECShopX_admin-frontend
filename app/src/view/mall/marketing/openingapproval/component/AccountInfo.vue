  ...
  <div>
      ...
      <el-form :model="form" :rules="rules" ref="form">
        <el-row class="cus-row-form">
          <el-col :span='12'>
            ...
            <el-select v-model="form.adapay_fee_mode" :clearable='true' placeholder="请选择" style="width: 100%;">
          ...
          <el-col :span='12'>
            <el-form-item label="总部分账占比" prop='headquarters_proportion' >
              <el-input v-model="form.headquarters_proportion" :clearable='true' placeholder="请输入" style="width: 100%;">
                <span slot="suffix">%</span>
              </el-input>
            </el-form-item>
          </el-col>
          <el-col :span='12' v-if="info.is_rel_dealer || info.entry_apply_info.apply_type === 'dealer'">
            <el-form-item label="经销商分账占比" prop='dealer_proportion'>
              <el-input v-model="form.dealer_proportion" :clearable='true' placeholder="请输入" style="width: 100%;">
                <span slot="suffix">%</span>
              </el-input>
            </el-form-item>
          </el-col>
    ...
    >
      ...
      <div slot="footer" class="dialog-footer">
        <loading-btn ref="loadingBtn" size='medium' type='primary' text='确 定' @clickHandle="handleDialogChange('loadingBtn')" /> 
      </div>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        headquarters_proportion: [{ required: true, validator: this.validateNumber, trigger: "blur" }],
  ...
  methods: {
    handleDialogChange (ref) { // 点击审批弹框确定按钮
      ...
      setDetailApprove({
        ...
        save_id: entry_apply_info.apply_type === 'dealer' ? dealer_info.operator_id : distributor_info.distributor_id
      }).then((res) => {
    ...
    handleDialogOpen (formName, status) { // 打开审批弹框
     const { entry_apply_info: { user_name } } = this.info
    ...
    handleDialogClose () { // 关闭审批弹框
      this.dialogFormVisible = false
      this.comments = ''
      this.visibleContent = ''
    },
    validateNumber(rule, value, callback) { // 经销商分账占比校验
      const { headquarters_proportion, dealer_proportion } = this.form
      const { entry_apply_info: { apply_type }, is_rel_dealer } = this.info
      let sum = ''
      if (apply_type === 'dealer' || is_rel_dealer) { // 申请类型为店铺并且关联经销商
      ...
      if (!value) {
        ...
        } else if (Number(value) > 100){ // 店铺类型为直营店的时候 能输入100
