<template>
  <div>
    <div class="cus-form-label">
      <el-input placeholder="输入店铺名称" v-model="name" clearable class="cus-width-35">
        <el-button slot="append" icon="el-icon-search" @click="handleIconClick"></el-button>
      </el-input>
      <el-input placeholder="输入经营地址" v-model="address" clearable class="cus-width-35">
        <el-button slot="append" icon="el-icon-search" @click="handleIconClick"></el-button>
      </el-input>
    </div>
    <el-table
    ...
    >
      <el-table-column prop="name" label="店铺名称">
        <template slot-scope="scope">
          <i class="el-icon-house icons-size"></i>
          <span style="margin-left: 10px">{{ scope.row.name }}</span>
        </template>
      </el-table-column>
      <el-table-column prop="address" label="经营地址" show-overflow-tooltip>
        <template slot-scope="scope">
          <i class="el-icon-location-outline icons-size"></i>
