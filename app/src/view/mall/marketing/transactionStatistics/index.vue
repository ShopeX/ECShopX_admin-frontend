...
<!-- 6.30 -->
  <div class="page-transactionStatistics">
    <el-card class="box-card" shadow="never">
      ...
      <div class="content">
        <div class="item">
          <p class="money">{{ incomeList.income }}</p>
          <p>平台总计收入（含退款）</p>
        </div>
        <div class="item">
          <p class="money">{{ incomeList.refund }}</p>
          <p>平台总退款金额</p>
        </div>
        <div class="item">
          <p class="money">{{ incomeList.unsettled_funds }}</p>
          <p>平台未结算金额</p>
        </div>
        <div class="item">
          <p class="money">{{ incomeList.settlement_funds }}</p>
          <p>平台已结算金额</p>
        </div>
        <div class="item">
          <p class="money">{{ incomeList.withdrawal_balance }}</p>
          <p>当前可提现余额</p>
        </div>
      ...
      <div class="content2">
        <el-table :data="tableData" style="width: 100%" v-loading='loading'>
          ...
          <el-table-column prop="withdrawal_balance" label="当前可提现金额">
            <template slot-scope="scope">
              {{
                ...
                formatMoney(scope.row.withdrawal_balance / 100)
              }}
            </template>
          </el-table-column>
          ...
          <el-table-column prop="order_total_fee" label="总交易金额">
            <template slot-scope="scope">
              {{
                scope.row.order_total_fee &&
                formatMoney(scope.row.order_total_fee / 100)
              }}
            </template>
          </el-table-column>
...
export default {
  ...
  methods: {
    ...
    changeDateHandle() {
      if(this.dateValue){
        let timeDay = (new Date(this.dateValue[1]) - new Date(this.dateValue[0])) / (1000 * 60 * 60 * 24);
        if (timeDay>90) {
