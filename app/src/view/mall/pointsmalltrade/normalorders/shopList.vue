        ...
        <el-table :data="list" style="width: 100%" :height="wheight-190" v-loading="loading" element-loading-text="数据加载中">
          ...
          <el-table-column label="操作" fixed="right">
            <template slot-scope="scope">
              ...
              <el-button type="text" v-if="scope.row.cancel_status=='NO_APPLY_CANCEL' && (scope.row.order_status == 'NOTPAY' || scope.row.order_status == 'PAYED')" @click="cancelOrderAction(scope.row.order_id)">取消订单</el-button>
              <el-button type="text" v-if="scope.row.receipt_type=='logistics' && scope.row.order_status == 'PAYED' && scope.row.delivery_status != 'DONE'" @click="deliveryAction(scope.row.order_id)">发货</el-button>
...
<script>
  ...
  import { getOrderList, getOrderDetail, delivery, orderExport, getCancelOrderInfo, cancelConfirm, getLogisticsList, cancelOrderConfirm } from '../../../../api/trade'
  ...
  export default {
    ...
    methods: {
      ...
      deliveryAction (order_id) { // 编辑物料弹框
      ...
      confirmCancelOrderAction (order_id) { // 已支付订单的取消订单审核
      ...
      cancelOrderAction(order_id) { //取消订单
          let params = {order_type:'normal'}
          getOrderDetail(order_id).then(response => {
