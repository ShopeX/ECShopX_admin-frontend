...
-->
<template>
  <el-table class="template_table" :data="dataSource" border style="width: 100%" header-row-class-name='header_row_class'> 
    <el-table-column prop="date"  >
      <template slot="header" slot-scope="slot">
        <div class="first_header"><span>积分区间</span><span>*</span><span>最多添加5个区间</span></div>
      </template> 
    ...
    <el-table-column prop="name" label="操作" width="200">
        <template slot="header" slot-scope="slot">
          <div class="second_header"><span>操作</span></div>
        </template> 
        <template slot-scope="scope">
            <a @click="handleDelete(scope)" :class="{'disabled':dataSource.length===1}">删除</a>
        </template>
    </el-table-column> 
    <template slot="append" v-if="dataSource.length<5">
        <div class='footer' :class='{"noValue":!dataSource.length}'><span @click="handleAddScore">添加积分筛选区间</span></div>
    </template>
  </el-table>
</template>
...
.template_table {
  ...
  .el-table__append-wrapper,.el-table__body-wrapper{
    border-bottom-left-radius:6px;
    border-bottom-right-radius:6px;
  }
