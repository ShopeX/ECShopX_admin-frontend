<template>
    <div>
        <div v-if="$route.path.indexOf('editor') === -1">
            <el-row :gutter="20">
                <el-col :md="4" :lg="8">
                    <el-button size="mini" type="primary" icon="plus" @click="addActivityData">添加会员优先购活动</el-button>
                </el-col>
            </el-row>
                    ...
                    <el-table-column label="操作" width="150">
                        <template slot-scope="scope">
                            <div class="operating-icons">
                                <el-button @click="updateStatusCommunityAction(scope.row)" type="text"
                                           v-if="scope.row.status == 'ongoing'">取消
                                </el-button>
                                <el-button @click="viewDetail(scope.row)" type="text">查看</el-button>
                                <el-button @click="updateDetail(scope.row)" type="text"
                                           v-if="scope.row.status == 'waiting'">编辑
                                </el-button>
                            </div>
                        </template>
                    </el-table-column>
...
<script>
  ...
  import { getMarketingActivityList, getMarketingActivityItemList, removeMarketingActivity } from '../../../../api/promotions'
