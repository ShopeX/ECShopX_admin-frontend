...
<template>
  <div class="cus-authory-approve">
    ...
    <el-row class="cus-btn-box">
      <el-button v-if="approvel_info.status == '0'" type='success' class="btn" @click="handleDialogOpen(1)">通过</el-button>
      <el-button v-if="approvel_info.status == '0'" type='danger' class="btn" @click="handleDialogOpen(2)">驳回</el-button>
      <el-button v-if="approvel_info.status == '1' && approvel_info.is_closed == 0" :disabled="disabledBtn" type='primary' class="btn" @click="handleDialogOpen(3)">关闭权限</el-button>
      <el-button v-if="approvel_info.status == '1' && approvel_info.is_closed == 1" :disabled="disabledBtn" type='primary' class="btn" @click="handleDialogOpen(4)">打开权限</el-button>
    </el-row>
    <el-dialog
      title="审批弹框"
...
import { encryptDetail, datapassLog, encryptApprove, encryptClose, encryptOpen } from '@/api/encrypt'
...
export default {
  ...
  methods: {
    ...
    handleDialogOpen (status) { // 打开审批弹框
