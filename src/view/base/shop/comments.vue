...
<template>
  <div>
    <el-tabs v-model="activeName" class="section-white content-padded">
      <el-tab-pane :label="$t('f8a5bc8b.96e798')" name="first">
        <el-row :gutter="20">
          <el-col :span="12">
        ...
        <div>
          <el-table
            ...
            :data="list"
            style="width: 100%"
            :element-loading-text="$t('f8a5bc8b.f09b12')"
            :row-class-name="tableRowClassName"
          >
            <el-table-column prop="nickname" :label="$t('f8a5bc8b.5b0f22')" />
            <el-table-column prop="content" :label="$t('f8a5bc8b.034484')" />
            <el-table-column :label="$t('f8a5bc8b.d4fe37')">
            ...
            <el-table-column :label="$t('f8a5bc8b.a666ad')">
              <template slot-scope="scope">
                <span>{{ scope.row.created | datetime('YYYY-MM-DD HH:mm:ss') }}</span>
              </template>
            </el-table-column>
            <el-table-column :label="$t('f8a5bc8b.2b6bc0')">
              <template slot-scope="scope">
                <el-button size="small" @click.native.prevent="hid(scope.row)">
                  <span v-if="scope.row.hid">{{ $t('f8a5bc8b.59c094') }}</span
                  ><span v-else>{{ $t('f8a5bc8b.dce537') }}</span>
                </el-button>
                <el-button size="small" @click.native.prevent="top(scope.row)">
                  <span v-if="scope.row.stuck">{{ $t('f8a5bc8b.84e4fa') }}</span
                  ><span v-else>{{ $t('f8a5bc8b.3d43ff') }}</span>
                </el-button>
              </template>
            </el-table-column>
          </el-table>
