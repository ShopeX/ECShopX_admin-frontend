...
import { deleteSmsTemplate } from '@/api/tempSms.js'
export default {
  data() {
    return {
      failVisible: false,
  ...
  computed: {
    setting() {
      const res = setting_(this)
      return {
        ...setting_(this),
        search: [{ key: 'template_name', name: '模板名称' }],
        columns: res.columns.filter((el) => el.key !== 'template_code' && el.key !== 'status')
      }
    }
  },
  methods: {
