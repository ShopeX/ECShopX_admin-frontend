...
export default {
  ...
  watch: {
    async 'form.template_type'(template_type) {
      ...
      const _data = await getSmsSignatureList({
        page: 1,
        pageSize: 500,
        sign_type: template_type
      })
      this.signNameList = _data?.data?.data?.list?.map((item) => ({
  ...
  methods: {
    ...
    fnBack() {
      this.$router.push({
        path: `/setting/datamessage/tmp_sms/sms_template`
      })
    },
    fnAgain() {
