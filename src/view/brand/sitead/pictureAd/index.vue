<template>
  <div class="page-body">
    <SpRouterView>
      ...
      <SpFinder ref="finder" url="/adplace/list" fixed-row-action row-actions-width="200px" :split-count="5"
        :setting="tableSchema" :hooks="{
          beforeSearch
        }" no-selection>
        <template v-slot:tableTop>
          <div class="action-container">
            <div style="height: 20px" />
            <el-button type="primary" plain @click="addHandle">创建广告</el-button>
      ...
      <SpDialog ref="dialogRef" v-model="dialogShow" :title="!editRow ? '添加广告' : '编辑广告'" :modal="false"
        class="base-form" :form="activityRule" :form-list="activityRuleList" @onSubmit="onFormSubmit" />
    </SpRouterView>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    async onFormSubmit() {
      ...
      if (typeof _baseParams.distributor_id === 'string') {
        _baseParams.distributor_id = _baseParams.distributor_id.split(',')
      }
    ...
    editHandle(row) {
      ...
      this.activityRule = {
        ...
        distributor_id: row.rel_distributors?.map((el) => el.distributor_id)
      }
      try {
    ...
    async editDistributor(row) {
      console.log(row, 'row')
      const {rel_distributors,name,regionauth_id,start_time,end_time,sort,pages,ad_type,setting,id} = row
      const distributorIds = rel_distributors.map((item) => item.distributor_id)
      ...
      // if (this.editRow) {
        await api.wxastats.updatePictureAd(id, _baseParams)
      // } else {
      //   await api.wxastats.createPictureAd(_baseParams)
      // }
      this.refresh()
