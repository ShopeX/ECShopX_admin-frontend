<template>
    <div class="attr-data">
        <!-- 销售分类 -->
        <CompButton v-if="type === 'category'" :placeholder="value?.info?.title || '选择销售分类'" :view-btn="false"
            @click="handleSelectCategory" />

        <!-- 管理分类 -->
        <CompButton v-if="type === 'main_category'" :placeholder="value?.info?.title || '选择管理分类'" :view-btn="false"
            @click="handleSelectMainCategory" />

        <!-- 限时特惠活动 -->
        <CompButton v-if="type === 'seckill'" :placeholder="value?.info?.title || '请选择限时特惠活动'" :view-btn="false"
        ...
        <!-- 品类热销榜单 -->
        <CompButton v-if="type === 'category_ranking'" :placeholder="value?.info?.title || '选择榜单'" :view-btn="false"
            :clear-btn="false" @click="handleSelectRanking" />

        <!-- 平台品类榜单 -->
        <CompButton v-if="type === 'platform_ranking'" :placeholder="value?.info?.title || '选择平台榜单'" :view-btn="false"
            :clear-btn="false" @click="handleSelectPlatformRanking" />

        <!-- 价格区间 -->
        <div v-if="type === 'price'" class="price-range-container">
            <el-input-number v-model="minPrice" :min="0" :precision="0" size="small" :max="maxPrice || 999999"
                placeholder="最低价" controls-position="right" @change="handleChangePrice" />
            <span class="separator">至</span>
            <el-input-number v-model="maxPrice" :min="minPrice || 0" :precision="0" size="small" placeholder="最高价"
                controls-position="right" @change="handleChangePrice" />
        </div>

        <!-- 活动集合页-秒杀 -->
        <CompButton v-if="type === 'promotion'" :placeholder="value?.info?.name || '选择秒杀活动'" :view-btn="false"
            @click="handleChangePromotion" />

        <!-- 活动集合页-拼团 -->
        <CompButton v-if="type === 'group'" :placeholder="value?.info?.name || '选择拼团活动'" :view-btn="false"
...
export default {
    ...
    methods: {
        ...
        async handleSelectMainCategory(v) {
            ...
            this.$emit('input', { id: data[0].id, info: { regionauth_id: this.$route.query.regionauth_id, title: data[0].title } })
        },
        // 选择销售分类
        async handleSelectCategory(v) {
            ...
            this.$emit('input', { id: data[0].id, info: { regionauth_id: data[0].regionauth_id, title: data[0].title } })
        },
        // 选择限时特惠活动
        async handleSelectSeckill() {
            ...
            this.$emit('input', { id: data[0].seckill_id, info: { name: data[0].collection_name, regionauth_id: data[0].regionauth_id } })
        },
        ...
        async handleSelectGroup() {
            ...
            this.$emit('input', { id: data[0].collection_id, info: { name: data[0].collection_name, regionauth_id: data[0].regionauth_id } })
        },
        // 选择价格区间
        handleChangePrice() {
