<template>
  <div class="wgt-content-partition">
    <!-- 导航栏 -->
    <div class="nav-wrapper" :style="navWrapperStyle">
      <!-- 左滑示意图 -->
      <img
        v-if="value.leftimgUrl && showLeftIndicator"
      ...
      <div ref="navScroller" class="nav-scroller" :style="navItemAreaStyle" @scroll="handleScroll">
        <div class="nav-list" :style="navListStyle">
          ...
          >
            ...
            <!-- 选中底线 -->
            <span
              v-if="value.navitemborder && activeIndex === index"
      ...
      <!-- 右滑示意图 -->
      <img
        v-if="value.rightimgUrl && showRightIndicator"
    ...
    <div class="content-wrapper">
      <!-- 当前选中项的内容区块 -->
      <div v-if="activeNavItem" :id="`nav-section-${activeIndex}`" class="nav-section">
        <template v-if="activeNavItem.children && activeNavItem.children.length > 0">
          <component
            v-for="(child, idx) in activeNavItem.children"
            :key="`widget-${activeIndex}-${idx}`"
...
export default {
  ...
  wgtName: i18n.t('dfa780ca.b3876f'),
  wgtDesc: '',
  wgtIcon: 'app-switch',
  config: config,
  props: {
  ...
  computed: {
    ...
    navWrapperStyle() {
      ...
      // 根据 bgType 处理背景
      if (bgType === 'gradient') {
        backgroundImage = `linear-gradient(to bottom, ${outerMargin.startColor || ''}, ${
          outerMargin.endColor || ''
        })`
      } else if (bgType === 'pic') {
    ...
    navItemAreaStyle() {
      ...
      // 根据 bgType 处理背景
      if (bgType === 'gradient') {
        backgroundImage = `linear-gradient(to bottom, ${navitemarea.startColor || ''}, ${
          navitemarea.endColor || ''
        })`
      } else if (bgType === 'pic') {
  ...
  methods: {
    ...
    getNavItemStyle(item, index) {
      ...
      const navitemmargin = this.value.navitemmargin || 0
      return {
        height: `${this.value.navitemheight || 40}px`,
