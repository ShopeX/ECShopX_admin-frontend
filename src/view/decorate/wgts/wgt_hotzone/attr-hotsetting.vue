    ...
    <el-dialog :visible="dialog" class="sp-dialog" append-to-body destroy-on-close title="热区设置" width="800px"
      ...
      <div v-if="dialog" class="">
        ...
        <div class="hot-content">
          <el-row :gutter="20">
            <el-col :span="12">
              <hotzone class="hotzone" :image="value.imgUrl" :zones-init="value.data" @change="handleChange"
                @remove="handleRemove" />
            </el-col>
            <el-col :span="12">
              <div v-for="(item, index) in value.data" :key="`zone-item__${index}`" class="zone-item">
                <CompPickerLink :value="item" :regionauthid="regionauthid" @change="(e) => onChangeLink(e, index)" />
              </div>
            </el-col>
          </el-row>
        </div>
