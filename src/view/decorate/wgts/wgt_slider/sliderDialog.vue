...
<template>
    <div class="wgts-slider-dialog">
        <div class="wgts-slider-dialog-left" v-if="sliderForm.media_type === 'img'"
            ...
            <SpImage v-if="!sliderForm.imgUrl" :src="sliderForm.imgUrl" class="sp-image" fit="scale-down" />
            <hotzone v-if="sliderForm.imgUrl" class="hotzone" :image="sliderForm.imgUrl"
                :zones-init="sliderForm.hotData" :key="`hotzone-img-${hotzoneKey}`" @add="handleAdd($event)" @change="handleChange($event)"
                @remove="handleRemove($event, 'img')" />
            <hotzone class="overlay" :image="sliderForm.overlay || ''" :zones-init="sliderForm.overlayHotData"
                v-if="sliderForm.overlay" :key="`overlay-img-${overlayHotzoneKey}`" :style="overlayStyle"
                @add="handleAdd($event, 'overlay')" @change="handleChange($event, 'overlay')"
        ...
        <div class="wgts-slider-dialog-left" v-else :style="`height:${h}px;background:#f5f5f7`">
            ...
            <video-player v-if="videoUrlStr" class="image-item-video" :options="{
                aspectRatio: '16:9',
                fluid: true,
                sources: [
                    {
            ...
            <hotzone class="overlay" :image="sliderForm.overlay || ''" :zones-init="sliderForm.overlayHotData"
                v-if="sliderForm.overlay" :key="`overlay-video-${overlayHotzoneKey}`"
                :style="overlayStyle" @add="handleAdd($event, 'overlay')" @change="handleChange($event, 'overlay')"
                @remove="handleRemove($event, 'overlay')" />
        </div>
        <SpForm ref="form" class="sp-form" v-model="sliderForm" :form-list="sliderFormList" :submit="false" label-width="110px" />
    </div>
</template>
...
export default {
            ...
            setTimeout(() => {
                ...
                zone.forEach((item, index) => {
                    if (item && item.leftPer) {
                        ...
                        if (type == 'overlay') {
                            if (!this.sliderForm.overlayHotData || index >= this.sliderForm.overlayHotData.length || !this.sliderForm.overlayHotData[index]) return
                            ...
                            if (!this.sliderForm.hotData || index >= this.sliderForm.hotData.length || !this.sliderForm.hotData[index]) return
    ...
    computed: {
        ...
        videoUrlStr() {
            ...
            return typeof v === 'object' && v !== null && v.url != null ? v.url : (typeof v === 'string' ? v : '')
        },
        sliderFormList() {
            if (!this.sliderForm) return []
