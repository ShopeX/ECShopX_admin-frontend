  ...
  <div>
    <div v-if="$route.path.indexOf('_detail') === -1">
          ...
          <el-col :span="5" class="admin-total-num-box">
            ...
            <p class="admin-total-num-bot-fonts">
              ...
              <el-popover
                ...
                content='分账金额统计(不含退款)'
              >
                <i class="el-icon-question" style="color:#c1c1c1;font-size: 16px" slot="reference"></i>
              </el-popover>
            </p>
          </el-col>
          <el-col :span="5" class="admin-total-num-box">
          ...
          <el-row style='marginBottom: 0px'>
            ...
            <el-col :span="6">
              <el-form-item label="订单编号">
                <el-input size='small' v-model="formData.order_id" placeholder="请输入订单编号"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="6">
            ...
            <el-col :span="12" v-if="loginType !== 'distributor'">
              <el-form-item label="选择店铺">
                <shop-select
                  ...
                  @update="storeSearch"
                  :shopIdDefault="formData.distributor_id"
            ...
            <el-col :span="4" :push='14' v-if="loginType === 'distributor'" style="margin-left:10px">
      ...
      <el-card>
        <el-row style='marginBottom: 0px'>
          <el-col :span="3" class="total-num-box">
            <p class="total-num-top-fonts">{{allAccountList.order_count}}</p>
          ...
          <el-col :span="3" class="total-num-box">
            <p class="total-num-top-fonts">{{allAccountList.order_refund_count}}</p>
          ...
          <el-col :span="3" class="total-num-box">
            <p class="total-num-top-fonts">{{allAccountList.order_refunding_count}}</p>
      ...
      <el-card>
        ...
        <el-table :data="tableDataList" stripe border style="width: 100%" v-loading="loading">
          ...
          <el-table-column prop="app_pay_type_desc" label="支付类型" width="150" v-if="loginType === 'distributor'"></el-table-column>
          <el-table-column prop="profitsharing_status" label="结算状态">
            <template slot-scope="scope">
              {{ (scope.row.profitsharing_status === '1' || scope.row.profitsharing_status === 1) ? '未结算' : '已结算' }}
            </template>
          </el-table-column>
          <el-table-column label="交易金额(含退款)" width="170">
            <template slot-scope="scope"><p style="color:#67C23A">￥{{(scope.row.total_fee/100).toFixed(2)}}</p></template>
          </el-table-column>
          ...
          <el-table-column label="退款金额" width="160">
            <template slot-scope="scope"><p style="color:#D9001B">￥{{(scope.row.refund_fee/100).toFixed(2)}}</p></template>
          </el-table-column>
          ...
          <el-table-column
            ...
            width="100">
            <template slot-scope="scope">
            <router-link :to="{ path: matchHidePage('detail'), query: { orderId: scope.row.order_id}}">详情</router-link>
            </template>
          </el-table-column>
        </el-table>
...
<script>
  ...
  export default {
    ...
    methods: {
      getCompanyDataList () { // 搜索
        this.dataSource.loading = true;
        GetAccountStatisList({...this.formData, ...this.params }).then(res => {
