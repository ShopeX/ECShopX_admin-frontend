  ...
  >
    ...
    >
      <el-table-column
        ...
        width="80"
      />
      <el-table-column prop="itemId" :label="$t('d41d8cd9.858526')" />
      <el-table-column prop="itemName" :label="$t('d41d8cd9.1fd1d5')" />
      <el-table-column prop="itemBn" :label="$t('d41d8cd9.f9d8b1')" />
      <el-table-column
        prop="price"
        :label="$t('d41d8cd9.0e9fd9')"
        width="80"
    ...
    <span slot="footer" class="dialog-footer">
      <el-button @click="cancelAction">{{ $t('d41d8cd9.c08ab9') }}</el-button>
      <el-button type="primary" @click="saveGoodsAction">{{ $t('d41d8cd9.aa7527') }}</el-button>
    </span>
  </el-dialog>
</template>
...
export default {
  ...
  methods: {
    ...
    handleSelectAll(val) {
      if (this.limitNum) {
        this.$message({ message: this.$t('d41d8cd9.4f180d'), type: 'warning' })
        this.$refs.multipleTable.clearSelection()
    ...
    handleSelectChange(val, row) {
      ...
      if (inChecked !== -1) {
        ...
        if (this.limitNum && this.limitNum == 1) {
          ...
          this.$message({
            message: `${this.$t('d41d8cd9.a0672e')}${this.limitNum}${this.$t('d41d8cd9.777ba9')}`,
            type: 'warning'
          })
          this.$refs.multipleTable.clearSelection()
          this.selectRows.forEach((item) => {
