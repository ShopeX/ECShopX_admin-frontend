...
<template>
  <div>
    ...
    <el-table v-loading="loading" :data="tagsList" :height="wheight - 130">
      ...
      <el-table-column prop="tag_name" :label="$t('8268047b.341fe8')" width="250">
      ...
      <el-table-column prop="description" :label="$t('8268047b.da5bf4')" width="250" />
      <el-table-column :label="$t('8268047b.2b6bc0')">
        <template slot-scope="scope">
    ...
    >
      <template>
        <el-form ref="form" :model="form" class="demo-ruleForm" label-width="100px">
          <el-form-item :label="$t('8268047b.abd688')">
          ...
          <el-form-item
            class="content-left"
            :label="$t('8268047b.341fe8')"
            prop="tag_name"
            :rules="[{ required: true, message: $t('8268047b.6f81f3'), trigger: 'blur' }]"
          >
            <el-input v-model="form.tag_name" :placeholder="$t('8268047b.6f81f3')" />
          </el-form-item>
          <el-form-item class="content-left" :label="$t('8268047b.beff13')">
            <el-input
              v-model="form.description"
              type="textarea"
              :rows="3"
              :placeholder="$t('8268047b.92f284')"
            />
          </el-form-item>
          <el-form-item class="content-left" :label="$t('8268047b.0a6bfb')">
            <el-color-picker v-model="form.tag_color" show-alpha :predefine="predefineColors" />
          </el-form-item>
          <el-form-item class="content-left" :label="$t('8268047b.690660')">
            <el-color-picker v-model="form.font_color" show-alpha :predefine="predefineColors" />
          </el-form-item>
          <el-form-item class="content-center">
            <el-button type="primary" @click="saveTagData"> {{ $t('8268047b.b75381') }} </el-button>
          </el-form-item>
        </el-form>
      </template>
    </el-dialog>
    ...
    getDataList() {
      ...
      getTagList(this.params)
        ...
        .catch((error) => {
          ...
          this.$message({
            type: 'error',
            message: this.$t('8268047b.f40299')
          })
        })
    },
    deleteAction(index, row) {
      this.$confirm(this.$t('8268047b.38c88c'), this.$t('8268047b.02d981'), {
        confirmButtonText: this.$t('8268047b.38cf16'),
        cancelButtonText: this.$t('8268047b.625fb2'),
        type: 'warning'
      })
        .then(() => {
          deleteTag(row.tag_id)
            .then((response) => {
              ...
              this.$message({
                message: this.$t('8268047b.0007d1'),
                type: 'success',
                duration: 5 * 1000
              })
            })
            .catch(() => {
              this.$message({
                type: 'error',
                message: this.$t('8268047b.acf066')
              })
            })
        })
        .catch(() => {
          this.$message({
            type: 'info',
            message: this.$t('8268047b.2111cc')
          })
        })
    },
    getTaskTime(strDate) {
      ...
      if (this.form.tag_id) {
        updateTag(this.form).then((res) => {
          if (res.data.data) {
            this.$message({
              type: 'success',
              message: this.$t('8268047b.3b1083')
            })
            this.getDataList()
          }
        })
