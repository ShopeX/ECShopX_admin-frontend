    ...
    <div v-if="$route.path.indexOf('detail') === -1">
      <el-row class="filter-header" :gutter="20">
        <el-col>
          ...
          <el-autocomplete
            ...
            @select="sourceSearch"
          ></el-autocomplete>
          <el-select v-model="distributor_id" @change="distributorSelectHandle" placeholder="请选择店铺">
            <el-option
      ...
      <el-tabs type="border-card" v-model="activeName" @tab-click="handleClick">
        ...
        <el-table :data="list" style="width: 100%" :height="wheight-190" v-loading="loading" element-loading-text="数据加载中">
          <el-table-column prop="order_id" width="280" label="订单号">
          ...
          <el-table-column width="140" label="状态">
            <template slot-scope="scope">
              ...
              <el-tag v-if="scope.row.order_status == 'WAIT_BUYER_CONFIRM'" type="danger" size="mini">待收货</el-tag>
              <template v-if="scope.row.order_status != 'CANCEL'">
                <!-- 发货状态 -->
                <el-tag v-if="scope.row.delivery_status == 'DONE'" type="success" size="mini">已发货</el-tag>
                ...
                <el-tag v-else-if="scope.row.ziti_status == 'DONE'" type="danger" size="mini">已自提</el-tag>
          ...
          <el-table-column width="90" label="操作">
            <template slot-scope="scope">
              <el-button type="text">
                <router-link :to="{ path: matchHidePage('detail'), query: { orderId: scope.row.order_id, resource: '/mall/trade/normalorders' }}">详情</router-link>
              </el-button>
        ...
        <template>
          <el-form ref="deliveryForm" :model="deliveryForm" class="demo-ruleForm" label-width="90px">
            <el-form-item label="订单号">
              <el-col :span="20">{{deliveryData.orderInfo.order_id}}</el-input></el-col>
            </el-form-item>
