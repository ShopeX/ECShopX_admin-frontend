...
<template>
  <div>
    <div class="cus-form-label">
      <el-input v-model="name" :placeholder="$t('7a96e69f.240fdb')" clearable class="cus-width-35">
        <el-button slot="append" icon="el-icon-search" @click="handleIconClick" />
      </el-input>
      <el-input
        v-model="address"
        :placeholder="$t('7a96e69f.19ee5b')"
        clearable
        class="cus-width-35"
      >
        <el-button slot="append" icon="el-icon-search" @click="handleIconClick" />
    ...
    <el-table v-loading="info.loading" :data="data" tooltip-effect="dark" style="width: 100%">
      <el-table-column prop="name" :label="$t('7a96e69f.0d4934')">
        <template slot-scope="scope">
          <i class="el-icon-house icons-size" />
          <span style="margin-left: 10px">{{ scope.row.name }}</span>
        </template>
      </el-table-column>
      <el-table-column prop="address" :label="$t('7a96e69f.0ed872')" show-overflow-tooltip>
        <template slot-scope="scope">
