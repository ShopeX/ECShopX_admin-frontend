<template>
  <div>
    <el-row class="content-bottom-padded" :gutter="20">
      ...
      <el-col :span="6">
        <el-input placeholder="分销商手机号" v-model="params.mobile"
          ><el-button slot="append" icon="el-icon-search" @click="numberSearch"></el-button
        ></el-input>
      </el-col>
    </el-row>
    <el-table :data="list" height="550" border v-loading="loading">
      <el-table-column prop="created" label="申请时间" min-width="84">
        <template slot-scope="scope">
          <span>{{ scope.row.created | datetime('YYYY-MM-DD HH:mm:ss') }}</span>
        </template>
      </el-table-column>
      <el-table-column prop="distributor_name" label="分销商姓名"></el-table-column>
      <el-table-column prop="distributor_mobile" label="分销商手机号"></el-table-column>
      ...
      <el-table-column label="操作" width="160">
        <template slot-scope="scope">
          <el-button size="mini" type="info" v-if="scope.row.status == 'reject'" v-popover:popover>
