...
<template>
  <div>
    ...
    >
      ...
      <el-table-column prop="name" :label="$t('98a4aebd.621f21')" />
      <el-table-column :label="$t('98a4aebd.087003')">
        <template slot-scope="scope">
          {{ scope.row.area | formatCityData(district) }}
        </template>
      </el-table-column>
      <el-table-column prop="start_standard" :label="$t('98a4aebd.e56878')" />
      <el-table-column prop="start_fee" :label="$t('98a4aebd.23e30a')" />
      <el-table-column prop="add_standard" :label="$t('98a4aebd.1ed90e')" />
      <el-table-column prop="add_fee" :label="$t('98a4aebd.0dc2ad')" />
      <el-table-column :label="$t('98a4aebd.3fea7c')">
        <template slot-scope="scope">
          <span v-if="scope.row.status == true">{{ $t('98a4aebd.7854b5') }}</span>
          <span v-else>{{ $t('98a4aebd.b15d91') }}</span>
        </template>
      </el-table-column>
      <el-table-column :label="$t('98a4aebd.4b9676')">
        <template slot-scope="scope">
          <span>{{ scope.row.updated_at | datetime }}</span>
        </template>
      </el-table-column>
      <el-table-column :label="$t('98a4aebd.2b6bc0')" width="250">
        <template slot-scope="scope">
...
export default {
  ...
  methods: {
    ...
    deleteTemplatesAction(index, row) {
      this.$confirm(this.$t('98a4aebd.e0d827'), this.$t('98a4aebd.02d981'), {
        confirmButtonText: this.$t('98a4aebd.38cf16'),
        cancelButtonText: this.$t('98a4aebd.625fb2'),
        type: 'warning'
      })
        .then(() => {
          deleteShippingTemplates(row.template_id).then((response) => {
            ...
            this.$message({
              message: this.$t('98a4aebd.9f3fee'),
              type: 'success',
              duration: 5 * 1000
            })
          })
        ...
        .catch(() => {
          this.$message({
            type: 'info',
            message: this.$t('98a4aebd.2111cc')
          })
        })
    },
