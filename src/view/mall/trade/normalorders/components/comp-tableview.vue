...
<template>
  <div class="comp-tableview">
    <el-table
      ...
      border
    >
      <el-table-column prop="itemName" :label="$t('72034cff.9897d8')">
        <template slot-scope="scope">
          <div>{{ scope.row.item_name }}</div>
          <div class="sku-text">{{ scope.row.item_spec_desc }}</div>
        </template>
      </el-table-column>
      <el-table-column prop="price" :label="$t('72034cff.6bf871')" width="100" />
      <el-table-column prop="num" :label="$t('72034cff.0bf60b')" width="80" />
      <el-table-column prop="discount_fee" :label="$t('72034cff.feab3a')" width="100" />
      <el-table-column prop="point" :label="$t('72034cff.2ecbc8')" width="120" />
      ...
      <el-table-column prop="total" :label="$t('72034cff.c90e66')" width="150" />
    </el-table>
    <div class="tableview-ft">
      <div class="ft-l">
        <div class="l-item">
          <label>{{ $t('72034cff.61f98b') }}</label>
          <el-input
          ...
          />
          <el-button type="text" @click="handleChangePrice">
            {{ $t('72034cff.38cf16') }}
          </el-button>
        </div>
        <div class="l-item">
          <label>{{ $t('72034cff.94a6a5') }}</label
          ><el-input
        ...
        <div v-if="pointFreightFee" class="l-item">
          <div class="freight-point-discount">
            {{ $t('72034cff.70aecb') }}{{ `${pointFreightFee.toFixed(2)}` }}
          </div>
        </div>
      </div>
      <div class="ft-r">
        <div class="r-item">
          <label>{{ $t('72034cff.cd29e9') }}</label
          ><span>{{ ` ¥${dItemTotalFee.toFixed(2)}` }}</span>
        </div>
        <div class="r-item">
          <label>{{ $t('72034cff.95bac9') }}</label
          ><span>{{ ` ¥${dFreightFee.toFixed(2)}` }}</span>
        </div>
        <div class="r-item">
          <label>{{ $t('72034cff.70aecb') }}</label
          ><span>{{ `-¥${pointFreightFee.toFixed(2)}` }}</span>
        </div>
        <div class="r-item">
          <label>{{ $t('72034cff.8ebece') }}</label
          ><span>{{ ` ¥${dOrderFee.toFixed(2)}` }}</span>
        </div>
      </div>
    </div>
    <div class="dialog-tip">
      <p>{{ $t('72034cff.7601db') }}</p>
      <p>
        {{ $t('72034cff.4983fd') }}
      </p>
      <p>{{ $t('72034cff.56e179') }}</p>
    </div>
  </div>
</template>
    ...
    value(newVal, oldVal) {
      this.tableData = newVal.map((item) => {
        return {
          ...
          item_spec_desc: item.item_spec_desc || this.$t('72034cff.048df4'),
  ...
  methods: {
    ...
    onChangeItemPrice({ change_price, total_fee }) {
      if (parseFloat(change_price) >= parseFloat(total_fee)) {
        this.$message.error(this.$t('72034cff.1536c9'))
        return
      }
