  ...
  <div class="">
    <div v-if="$route.path.indexOf('detail') === -1">
      ...
      <el-row :gutter="10">
        <el-col :md="7" :lg="5">
          <el-autocomplete
            ...
            :placeholder="$t('a19ec3b5.4b525f')"
            size="mini"
            @select="sourceSearch"
          />
        </el-col>
        <el-col :md="7" :lg="5">
          <el-input v-model="salesman_mobile" :placeholder="$t('a19ec3b5.f1ba67')" size="mini">
            <el-button slot="append" icon="el-icon-search" @click="numberSearch" />
          </el-input>
        </el-col>
        <el-col :md="7" :lg="5">
          <el-input v-model="identifier" :placeholder="$t('a19ec3b5.f4b2e7')" size="mini">
            <el-button slot="append" icon="el-icon-search" @click="numberSearch" />
          </el-input>
        </el-col>
        <el-col :span="3">
      ...
      <el-tabs v-model="activeName" type="border-card" @tab-click="handleClick">
        <el-tab-pane :label="$t('a19ec3b5.a8b0c2')" name="all" />
        <el-tab-pane :label="$t('a19ec3b5.5cb424')" name="notpay" />
        <el-tab-pane :label="$t('a19ec3b5.25d532')" name="done" />
        <el-tab-pane :label="$t('a19ec3b5.2111cc')" name="cancel" />
        <el-table
          v-loading="loading"
          :data="list"
          style="width: 100%"
          :height="wheight - 190"
          :element-loading-text="$t('a19ec3b5.f09b12')"
        >
          <el-table-column prop="order_id" width="150" :label="$t('a19ec3b5.1e8dc2')" fixed />
          <el-table-column prop="create_time" width="160" :label="$t('a19ec3b5.eca37c')">
            <template slot-scope="scope">
              <span>{{ scope.row.create_time | datetime('YYYY-MM-DD HH:mm:ss') }}</span>
            </template>
          </el-table-column>
          <el-table-column prop="distributor_name" :label="$t('a19ec3b5.baad7e')">
            <template slot-scope="scope">
              <span v-if="scope.row.distributor_name">{{ scope.row.distributor_name }}</span>
              <span v-else>{{ $t('a19ec3b5.d166bb') }}</span>
            </template>
          </el-table-column>
          <el-table-column width="70" :label="$t('a19ec3b5.9a935b')">
            <template slot-scope="scope">
              {{ scope.row.fee_symbol }}{{ scope.row.freight_fee / 100 }}
            </template>
          </el-table-column>
          <el-table-column prop="total_fee" width="70" :label="$t('a19ec3b5.4cf24a')">
            <template slot-scope="scope">
              {{ scope.row.fee_symbol }}{{ scope.row.total_fee / 100 }}
            </template>
          </el-table-column>
          <el-table-column prop="mobile" width="110" :label="$t('a19ec3b5.8098e2')" />
          <el-table-column prop="order_status" :label="$t('a19ec3b5.86f6cf')">
            <template slot-scope="scope">
              <!-- 订单状态 -->
              ...
              <template v-if="scope.row.order_status != 'CANCEL'">
                ...
                <el-tag v-else type="primary" size="mini"> {{ $t('a19ec3b5.5cb424') }} </el-tag>
              </template>
            </template>
          </el-table-column>
          <el-table-column prop="source_name" :label="$t('a19ec3b5.26ca20')" />
          <el-table-column :label="$t('a19ec3b5.2b6bc0')" fixed="left">
            <template slot-scope="scope">
              <router-link
                :to="{
  ...
  methods: {
    ...
    exportData() {
      ...
      if (this.params.order_type != 'normal') {
        this.$message({
          type: 'error',
          message: this.$t('a19ec3b5.08bd5c')
        })
        return
      }
      orderExport(this.params).then((response) => {
        if (response.data.data.status) {
          this.$message({
            type: 'success',
            message: this.$t('a19ec3b5.3e1ddd')
          })
          return
          ...
          this.$message({
            type: 'error',
            message: this.$t('a19ec3b5.89ae53')
          })
          return
        }
      })
