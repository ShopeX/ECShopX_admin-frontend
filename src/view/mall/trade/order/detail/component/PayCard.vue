  ...
  methods: { 
    ...
    memberDiscountFilter() {
      return `- ${this.orderInfo.fee_symbol}${
        (this.orderInfo.member_discount / 100).toFixed(2)
      }`;
    },
    couponsReductionFilter() {
      return `-${this.orderInfo.fee_symbol}${
        (this.orderInfo.coupon_discount / 100).toFixed(2)
      }`;
    },
    disCountTotalMoney() {
      return `-${this.orderInfo.fee_symbol}${
    ...
    realTotalFilter() {
      ...
      if (this.tradeInfo.payType === "point") {
        ...
        returnValue = `${this.orderInfo.fee_symbol}${
          (this.orderInfo.total_fee / 100).toFixed(2)
        }`;
      }
      return returnValue;
    },
