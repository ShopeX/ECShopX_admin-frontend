  ...
  <el-form ref="form" :model="form" class="box-set" label-width="120px">
    <el-card header="基础信息" shadow="naver">
      ...
      <el-form-item
        label="是否支持叠加优惠券"
        prop="is_use_coupon"
        :rules="{ required: true, message: '是否支持叠加优惠券', trigger: 'blur' }"
      >
    ...
    <el-card header="适用范围" shadow="naver">
      ...
      <div v-if="!zdItemHidden" style="position: relative">
        <SkuSelector :filter-area="true" :data="relItems" :area-id="form.regionauth_id" @change="getItems" />
          ...
          <el-table v-if="relStores.length > 0" :data="relStores" style="line-height: normal">
            ...
            <el-table-column label="操作" width="50">
              <template slot-scope="scope">
                <el-button type="text" @click="deleteStoreRow(scope.$index, form.items)"
                  >删除</el-button
                >
              </template>
            </el-table-column>
...
export default {
  ...
  methods: {
    ...
    async onRejectFormSubmit() {
      console.log(111, this.rejectForm)
    ...
    async submitActivityAction() {
      ...
      thisform.tag_list = null //后台不需要
      thisform.brand_list = null //后台不需要
      ...
      if (this.form.marketing_id) {
        ...
        updateMarketingActivity(thisform)
          .then((res) => {
    ...
    getActivityDetail(id) {
      getMarketingActivityInfo({ marketing_id: id }).then((res) => {
        ...
        //  // 特定人群
        //  this.$api.member.getTagList({tag_id: response.member_tag_ids?.join(','),status:'1',is_onlyactive:'1'}).then((res) =>{
        //   this.form.crowd_ids = res.list
        // })
      })
    },
