  ...
  <div>
    <div v-if="$route.path.indexOf('editor') === -1">
      <el-row>
        ...
        <el-col :span="16">
          <div class="tips">
            用户发表的笔记会通过「机器审核」，若包含违禁内容将自动变为下架状态；官方笔记不会被审核，默认下架状态，需人工上架笔记；</br>若通过「机器审核」，则会为待审核，等待人工审核，此时社区内会展示笔记。
          </div>
        </el-col>
        <el-col :span="6">
          <el-input
...
import { getNotes,notesVerify,notesSetting,notesDelete,getBadge,getUGCSetting,setUGCSetting,batchSetBadges } from '@/api/ugc'
...
export default {
  ...
  components: {
      noteslist,auditNote,viewNotes,flagModal,settingOfficial
  },
  data () {
    return {
      activeName: 'first',
  ...
  methods: {
    ...
    getVideoSetting () {
      getUGCSetting({type:'video'}).then(res=>{
        var {data} = res.data;
        var video_enable = data['video.enable']=='1';
        this.$data.video_enable = video_enable;
      })
    },
    setUGCSetting () {
      const {video_enable} = this.$data;
      var params = {type:'video',setting:{'video.enable':video_enable?'1':'0'}};
...
.tips{width: 90%; color: #999; font-size: 12px; }
.barSelect{width: 100px;}
.marbor{display:inline-block;  margin:auto 15px; height:30px;vertical-align: middle;  border-left: 1px solid #eee;}
.searSelect{margin-right: 5px; width: 120px;}
.margin-col /deep/ .el-switch__label  span{color: #999;}
</style>
