    ...
    <section class="section section-white content-padded view-flex view-flex-middle">
      ...
      <div class="content-right">
        <el-switch
          ...
          active-color="#ff5000"
          :inactive-text="$t('d84f0154.554003')"
        />
        <el-switch
          v-model="settings.recharge"
          class="content-right-margin"
          active-color="#ff5000"
          :inactive-text="$t('d84f0154.0ae749')"
        />
        <el-button type="primary" @click="onSubmit"> {{ $t('d84f0154.be5fbb') }} </el-button>
      </div>
    </section>
      ...
      <div class="template-view">
        <div class="template" :style="'background:' + theme.bodyColor">
          ...
          <div class="section section-white nearly-store">
            <div class="section-header content-center">{{ $t('d84f0154.81b37c') }}</div>
        ...
        <div v-if="data.news" class="msgSender_content">
          ...
          >
            <div class="msg-content">
              <div class="msg-info">
                <span
                  >{{ $t('d84f0154.3a6d9c') }} {{ data.news.content.update_time | datetime }}</span
                >
              </div>
              <div
                class="sub-msg-item"
              ...
              >
                ...
                <a :href="data.news.content.news_item[0].url" class="edit-mask preview-mask">
                  <div class="edit-mask-content">{{ $t('d84f0154.f9d706') }}</div>
                </a>
        ...
        <div class="">
          <div class="appmsg_preview_area">
            <div class="appmsg_preview_container">
              ...
              <div class="appmsg_container_hd">
                <h4 class="appmsg_container_title">{{ $t('d84f0154.8eba89') }}</h4>
              </div>
                ...
                <div class="appmsg multi has_first_cover editing">
                  <div id="js_appmsg_preview" class="appmsg_content">
                    <template v-for="(item, index) in articals">
                      ...
                      >
                        <div class="first_appmsg_item" :title="$t('d84f0154.ff5ad1')">
            ...
            <template v-for="(item, index) in articals" v-if="id === index">
              <el-form ref="item" :model="item" label-position="top" label-width="80px">
                <div class="content-padded">
                  <el-form-item :label="$t('d84f0154.32c65d')" style="height: 80px">
                    <el-input v-model="item.title" :placeholder="$t('d84f0154.96641a')" />
                  </el-form-item>
                ...
                <div class="content-padded appmsg_edit_highlight_area" style="height: 420px">
                  <h3 class="header-title">{{ $t('d84f0154.874d4d') }}</h3>
                  <div class="header_tips">
                    {{ $t('d84f0154.05eb24') }}
                    <span class="form-text-tip">{{ $t('d84f0154.9f9b2a') }}</span>
                  </div>
                  <el-form-item>
                    <div>
                      <el-button @click="addThumbPreview"> {{ $t('d84f0154.60dd12') }} </el-button>
                    </div>
                    <div
                      v-if="item.thumb_url"
                      class="cover_preview"
          ...
          <div class="appmsg_tpl_area">
            <div class="appmsg_tpl_container">
              <div class="appmsg_container_hd">
                <h4 class="appmsg_container_title">{{ $t('d84f0154.ea2da2') }}</h4>
              </div>
              <div class="appmsg_container_bd">
                <ul id="js_media_list" class="tpl_list">
                  <li class="tpl_item img" style="" @click="addImgPreview">
                    <i class="el-icon-picture" />{{ $t('d84f0154.20def7') }}
                  </li>
                </ul>
                <imgPicker
  ...
  methods: {
    ...
    addmsg() {
      if (this.articals.length > 3) {
        this.$message({ message: this.$t('d84f0154.df31d7'), type: 'error' })
        return
      }
      let baseMsgData = {
        title: '',
    ...
    onSubmit() {
      ...
      if (that.pageSettingId) {
        ...
        updateParamsById(param).then((response) => {
          this.$message({
            message: this.$t('d84f0154.6d2221'),
            type: 'success',
            duration: 5 * 1000
          })
        })
        ...
        setPageParams(param).then((response) => {
          this.$message({
            message: this.$t('d84f0154.e04b36'),
            type: 'success',
            duration: 5 * 1000
          })
        })
      ...
      if (that.commonSettingId) {
        ...
        updateParamsById(settingParam).then((response) => {
          this.$message({
            message: this.$t('d84f0154.36edd7'),
            type: 'success',
            duration: 5 * 1000
          })
        })
        ...
        setPageParams(settingParam).then((response) => {
          this.$message({
            message: this.$t('d84f0154.5c88a8'),
            type: 'success',
            duration: 5 * 1000
          })
        })
