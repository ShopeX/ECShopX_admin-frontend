  ...
  <transition name="el-fade-in-linear" mode="out-in">
    <div>
            ...
            <div v-if="editableData.length > 0">
              <div class="layout-view">
                <div class="parent-view">
                  ...
                  >
                    {{ item.name ? item.name : $t('223b0007.1c883a') }}
                  </div>
                </div>
            ...
            <draggable v-model="editableData" :options="dragOptions">
              <div v-for="(item, idx) in editableData" :key="idx" class="form-parent">
                <div class="form-item">
                  ...
                  <el-input v-model="item.name" :placeholder="$t('223b0007.1c883a')" />
      ...
      <storeList
        ...
        @change="handleChange"
      />
      <sideBar :visible.sync="storeVisible" :title="$t('223b0007.d78af2')" width="24">
...
export default {
  ...
  methods: {
    ...
    addSeries() {
      if (!this.seriesTitle) {
        this.$message({ message: this.$t('223b0007.385d26'), type: 'error' })
        return
      }
    ...
    removeTab(targetName) {
      this.$confirm(this.$t('223b0007.31d64e'), this.$t('223b0007.02d981'), {
        confirmButtonText: this.$t('223b0007.38cf16'),
        cancelButtonText: this.$t('223b0007.625fb2'),
        type: 'warning'
      })
        .then(() => {
    ...
    // 移除楼层
    remove(idx) {
      this.$confirm(this.$t('223b0007.b28efa'))
        .then((_) => {
    ...
    saveConfig() {
      ...
      savePageParams(param).then((res) => {
        if (res.data.data.status) {
          this.$message({
            message: this.$t('223b0007.3b1083'),
            type: 'success'
          })
        }
      })
