  ...
  <div class="wxapp-home">
    ...
    <div v-if="!isDistributorTemplate" class="shop-header">
      <div v-if="!VERSION_B2C && !VERSION_IN_PURCHASE" class="shop-left">
        <el-select v-model="regionauth_id" placeholder="请选择" @change="changeHomeRegion">
          <el-option v-for="item in regionauthList" :key="item.regionauth_id" :label="item.label" :value="item.value" />
        </el-select>
        <el-button type="primary" style="margin-left: 10px;" @click="addTemplate">添加模板</el-button>
      ...
      <div class="section-white mini-setting">
        <!-- <el-button type="text" style="margin-right: 10px" @click="
          () => {
    ...
    <el-row :gutter="20" class="template-list">
      <el-col v-for="(item, index) in templateList" :key="index" class="template-col" :xs="12" :sm="12" :md="8" :lg="6"
        :xl="4">
        <div class="template-item">
          ...
          <div v-if="!VERSION_B2C" class="template-common">
            ...
            <el-switch v-model="item.element_edit_status" :active-value="1" :inactive-value="2"
          ...
          <div class="time-wrap">
            <div v-if="item.timer_status == 2" class="no-time">
              ...
              <div class="picker-wrap">
                ...
                <span>设置模板切换时间</span>
                <el-date-picker v-model="item.timer_time" value-format="yyyy-MM-dd HH:mm:ss" type="datetime"
          ...
          <div class="option-btns">
            <span class="btn" @click="editTemplate(item)">编辑</span>
            <span class="btn" @click="copyTemplate(item.pages_template_id)">复制</span>
            <span v-if="!isDistributorTemplate" class="btn" @click="handleClickNav(item.pages_template_id)">导航</span>
            <span class="btn" @click="abandonTemplate(item.pages_template_id)">废弃</span>
          </div>
          <div v-if="!isDistributorTemplate" class="synchronize-btn" @click="synchronizeTemplateToShop(index)">
            同步模板至店铺
          </div>
        </div>
      </el-col>
    ...
    <DistributorSelect :store-visible="distributorVisible" :is-synchronize="isSynchronize" :is-valid="isValid"
      :rel-data-ids="relDistributors" :get-status="distributorStatus" :parent-params="{regionauth_id:regionauth_id}"
      @chooseStore="distributorChooseAction" @chooseAllStore="allDistributorChooseAction" @closeStoreDialog="closeDialogAction" />

    <MallDecoration :dialog-visible="templateVisible" :template-name="template_name" :rel-store="relStore"
      :show-like="configForm.is_open_recommend ? 1 : 2" :template-id="currTemplateId" @saved="closeDialog"
      @closeDialog="closeDialog" />

    <TemplatePreview :dialog-visible="previewVisible" :rel-store="relStore"
      :show-like="configForm.is_open_recommend ? 1 : 2" :template-id="currTemplateId" :tabs="tabs" usage="page"
      @saved="closePreviewDialog" @closeDialog="closePreviewDialog" />

    <SpDialog ref="templateDialogRef" v-model="templateDialog" :title="templateForm.pages_template_id ? '编辑模板' : '添加模板'"
      :form="templateForm" :form-list="templateFormList" @onSubmit="onTemplateFormSubmit" />

    <SpDrawer v-model="configDrawerShow" class="config-drawer" :title="'小程序配置'" :footer="false">
      <SpForm ref="configForm" v-model="configForm" class="config-form" :label-width="'100px'"
        :form-list="configFormList" :submit="false" />
    </SpDrawer>

    <SpDrawer v-model="navDrawerShow" class="nav-drawer" :title="'导航设置'" :width="650" @confirm="
      () => {
        this.$refs['navForm'].handleSubmit()
      }
    ">
      <SpForm ref="navForm" v-model="navForm" class="nav-form" :label-width="'0'" :form-list="navFormList"
        :submit="false" @onSubmit="onSubmitTabList" />
    </SpDrawer>
  </div>
</template>
                ...
                {this.navForm?.tabList?.map((item, index) => (
                  <div class='nav-item'>
                    <div class='nav-item-hd'>
                      ...
                      {item.pagePath == 'other_wxapp' && (
                        <div class="uploader-setting">
                          <div class="btn-linkpath" onClick={this.handleOtherWxappSelect.bind(this, item)}>
                            {item?.otherWxapp?.app_name ?? '请选择外部小程序'}
                          </div>
                        </div>
                      )}
                    </div>
  ...
  methods: {
    ...
    async getTemplateSetInfo() {
      const {
        ...
        regionauth_id: this.regionauth_id
      })
    ...
    async handleOtherWxappSelect(item) {
      const { data } = await this.$picker.wxapp({
        multiple: false,
        data: item?.otherWxapp?.wx_external_config_id?[item?.otherWxapp?.wx_external_config_id]:[]
      })
      this.$set(item, 'otherWxapp', data[0])
    },
    ...
    addTemplate() {
      ...
      this.$set(this.templateForm, 'regionauth_id', this.regionauth_id)
      this.templateDialog = true
    },
    modifyTemplate({ pages_template_id, template_title, template_pic, regionauth_id }) {
      this.templateForm = {
        pages_template_id,
        template_title,
        template_pic,
    ...
    async onTemplateFormSubmit() {
      const { pages_template_id, template_title, template_pic, regionauth_id } = this.templateForm
    ...
    editTemplate({ pages_template_id, regionauth_id }) {
      ...
      if (distributor_id > 0) {
        ...
        this.$router.push(`/wxapp/manage/decorate?id=${pages_template_id}&regionauth_id=${regionauth_id}`)
      }
    },
    ...
    onChangePagePath(index, value) {
      ...
      if (value != 'other_wxapp' && this.navForm.tabList[index]?.otherWxapp) {
        this.$delete(this.navForm.tabList[index], 'otherWxapp')
      }
    },
    async onSubmitTabList() {
      ...
      const otherWxappIndex = tabList.findIndex(item => item.name == "other_wxapp" && !item.otherWxapp)
      if (emptyIndex > -1) {
        return this.$message({
          message: '请选择自定义页面',
          type: 'error',
