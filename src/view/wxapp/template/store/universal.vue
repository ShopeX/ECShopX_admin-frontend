  ...
  <div>
    <section class="section section-white content-padded view-flex view-flex-middle">
      ...
      <div class="content-right">
        <el-button type="primary" @click="save">{{ $t('eebf5c65.be5fbb') }}</el-button>
      </div>
    </section>
    <section class="section-white view-warp">
      ...
      <div v-if="currentPage === 0" class="template-view">
        <div class="template" :style="'background:' + theme.bodyColor">
          ...
          <div class="view-flex view-flex-middle activity">
            ...
            <div class="activity-btn">{{ $t('eebf5c65.9c1b27') }}</div>
          </div>
          ...
          <div class="goods-list">
            <div class="list-title">
              <text class="title-txt">{{ $t('eebf5c65.1dc55a') }}</text>
            </div>
          ...
          <div class="store">
            <div class="store-more store-label">{{ $t('eebf5c65.a48948') }}</div>
      ...
      <div v-if="currentPage === 0" class="setting-view">
        <div class="section">
          <div class="section-header with-border with-bg">{{ $t('eebf5c65.12d44c') }}</div>
        ...
        <div class="section">
          <div class="section-header with-border with-bg">{{ $t('eebf5c65.7d174d') }}</div>
          <div class="section-body">
            <div v-for="(item, index) in form.hotGoods" class="setting-item hot">
              <div class="goods-select" @click="handleGoodsChange(index, 'hotGoods')">
                ...
                <div v-else class="content-center">
                  <i class="el-icon-link" />{{ $t('eebf5c65.3b9737') }}
                </div>
              </div>
            </div>
          </div>
      ...
      <div v-if="currentPage === 1" class="artical-setting-view">
        <div v-if="data.news" class="msgSender_content">
          ...
          >
            <div class="msg-content">
              <div class="msg-info">
                <span
                  >{{ $t('eebf5c65.3a6d9c') }} {{ data.news.content.update_time | datetime }}</span
                >
              </div>
              <div
                class="sub-msg-item"
              ...
              >
                ...
                <a :href="data.news.content.news_item[0].url" class="edit-mask preview-mask">
                  <div class="edit-mask-content">{{ $t('eebf5c65.f9d706') }}</div>
                </a>
        ...
        <div class="">
          <div class="appmsg_preview_area">
            <div class="appmsg_preview_container">
              <div class="appmsg_container_hd">
                <h4 class="appmsg_container_title">{{ $t('eebf5c65.8eba89') }}</h4>
              </div>
                ...
                <div class="appmsg multi has_first_cover editing">
                  <div id="js_appmsg_preview" class="appmsg_content">
                    <template v-for="(item, index) in articals">
                      ...
                      >
                        <div class="first_appmsg_item" :title="$t('eebf5c65.ff5ad1')">
                          <div class="cover_appmsg_item">
                            <h4 class="appmsg_title">
                              ...
                              >
                                ...
                                <template v-else>{{ $t('eebf5c65.32c65d') }}</template>
                              </a>
                            </h4>
                            <div
            ...
            <template v-for="(item, index) in articals" v-if="id === index">
              <el-form ref="item" :model="item" label-position="top" label-width="80px">
                <div class="content-padded">
                  <el-form-item :label="$t('eebf5c65.32c65d')" style="height: 80px">
                    <el-input v-model="item.title" :placeholder="$t('eebf5c65.96641a')" />
                  </el-form-item>
                  <el-form-item>
                ...
                <div class="content-padded appmsg_edit_highlight_area" style="height: 420px">
                  <h3 class="header-title">{{ $t('eebf5c65.874d4d') }}</h3>
                  <div class="header_tips">
                    {{ $t('eebf5c65.05eb24') }}
                    <span class="form-text-tip">{{ $t('eebf5c65.9f9b2a') }}</span>
                  </div>
                  <el-form-item>
                    <div>
                      <el-button @click="addThumbPreview">{{ $t('eebf5c65.60dd12') }}</el-button>
                    </div>
                    <div
                      v-if="item.thumb_url"
                      class="cover_preview"
          ...
          <div class="appmsg_tpl_area">
            <div class="appmsg_tpl_container">
              <div class="appmsg_container_hd">
                <h4 class="appmsg_container_title">{{ $t('eebf5c65.ea2da2') }}</h4>
              </div>
              <div class="appmsg_container_bd">
                <ul id="js_media_list" class="tpl_list">
                  <li class="tpl_item img" style="" @click="addImgPreview">
                    <i class="el-icon-picture" />{{ $t('eebf5c65.20def7') }}
                  </li>
                </ul>
                <imgPicker
    ...
    >
      <template>
        <el-transfer
          v-model="selectGoods"
          :titles="[$t('eebf5c65.437974'), $t('eebf5c65.543b04')]"
          :button-texts="[$t('eebf5c65.be4911'), $t('eebf5c65.93d369')]"
          :data="goodsList"
...
export default {
  ...
  data() {
    return {
      ...
      pages: [
        {
          imgurl: '@/assets/img/wxapp_member_universal.jpg'
        },
        {
          imgurl: '@/assets/img/wxapp_member_universal01.jpg'
        }
      ],
      demoimg: img,
      logo: logo,
  ...
  methods: {
    ...
    handleAddBanner() {
      ...
      if (this.form.bannerPicList.length > 4) {
        this.$message({
          message: this.$t('eebf5c65.1d1897'),
          type: 'error',
          duration: 5 * 1000
        })
      } else {
    ...
    handleSelectChange(value, direction, movedKeys) {
      if (value.length > 1) {
        this.$message({
          message: this.$t('eebf5c65.0a5544'),
          type: 'error',
          duration: 5 * 1000
        })
      }
    ...
    addmsg() {
      if (this.articals.length > 3) {
        this.$message({ message: this.$t('eebf5c65.df31d7'), type: 'error' })
        return
      }
      let baseMsgData = {
        title: '',
    ...
    save() {
      if (this.bannerPicListSetId) {
        ...
        updateParamsById(bannerParams).then((response) => {
          this.$message({
            message: this.$t('eebf5c65.3b1083'),
            type: 'success',
            duration: 5 * 1000
          })
        })
