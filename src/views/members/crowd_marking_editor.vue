  ...
  <SpPage class="page-audience-selection">
    <div class="flex mb-1">
      ...
      <div class="ml-5 flex-1">
        ...
        <el-input v-model="description" type="textarea" show-word-limit :rows="2" maxlength="100" placeholder="请输入人群说明" :disabled="!isEditMode" />
      </div>
    </div>
    <div class="flex min-h-[calc(100vh-200px)] gap-5">
      <!-- 左侧特征选择面板 -->
      <div class="w-[300px] border border-[#e4e7ed] rounded bg-white overflow-y-auto">
        <div class="px-4 py-2 flex justify-between items-center border-b border-[#e4e7ed] font-medium">
          <span>选择客户特征</span>
          <div>
            <el-button type="text" v-if="activeCollapse.length === 0" size="small" @click="expandAll">全部展开</el-button>
      ...
      <!-- 右侧表单区域 -->
       <div class="flex-1 overflow-hidden">
         <div class="border border-[#e4e7ed] rounded bg-white flex flex-col overflow-hidden h-[calc(100vh-300px)]">
           ...
           <div class="flex-1 overflow-y-auto p-4">
             ...
             <template v-for="category in characteristicCategories">
               <!-- 只显示有表单项的分类 -->
               <div
                 v-if="hasCategoryItems(category.type)"
                 :key="category.type"
                 class="mb-6"
               >
                 <!-- 分类标题 -->
                 <div class="mb-3 px-2">
                   <span class="text-base font-semibold text-[#303133]">{{ getCategoryTitle(category.type) }}</span>
                 </div>
                   ...
                   >
                     ...
                     <div class="p-4">
                      <CompForm
                        ...
                        :disabled="!isEditMode"
                        @update="(idx, updated) => handleCategoryTotalConditionUpdate(category.type, tcIndex, updated)"
                      />
                     </div>
                   </div>
                 </template>
                 ...
                 <div
                   ...
                   class="mb-4 border border-[#e4e7ed] rounded bg-[#fafafa]"
                 >
                   <div class="flex justify-between items-center px-4 py-3 border-b border-[#e4e7ed] bg-white">
           ...
           <div class="py-1.5 px-4 border-t border-[#e4e7ed] flex items-center bg-white">
             ...
             <el-button type="primary" size="small" :disabled="!isEditMode" @click="onSearchChange">查询</el-button>
           </div>
         </div>
         <div class="border border-[#e4e7ed] rounded bg-white flex flex-col overflow-hidden max-h-[calc(100vh-200px)] mt-4">
           <div class="px-4 py-2 border-b border-[#e4e7ed] font-medium">
             <span>选择人群标签</span>
           </div>
           <div class="flex-1 overflow-y-auto p-4">
             <el-button type="primary" size="small" @click="showTagDialog = true" :disabled="!isEditMode">选择人群标签</el-button>
...
export default {
  ...
  methods: {
    ...
    addCharacteristic(category, item) {
      ...
      if (category.total_condition && Array.isArray(category.total_condition) && category.total_condition.length > 0) {
    ...
    removeCharacteristic(index) {
      ...
      if (item && item.categoryType) {
        const hasOtherItems = this.formItems.some(formItem => formItem.categoryType === item.categoryType)
        if (!hasOtherItems) {
    ...
    getCategoryTotalCondition(categoryType, index = 0) {
      ...
      const category = this.characteristicCategories.find(cat => cat.type === categoryType)
      if (category && category.total_condition && 
    ...
    buildConditionsData() {
      return this.characteristicCategories.map(category => {
        ...
        if (category.total_condition && category.total_condition.length > 0) {
          ...
          category.total_condition.forEach((tc, index) => {
            if (tc && tc.condition_type) {
              ...
              if (categoryTotalCondition && categoryTotalCondition.value !== null && categoryTotalCondition.value !== undefined) {
                let valueArr = null
                ...
                if (conditionType === 'timeRange') {
                  if (Array.isArray(categoryTotalCondition.value)) {
                    valueArr = categoryTotalCondition.value.map(timestamp => dayjs(timestamp).unix())
                  }
                } else if (conditionType === 'number_rang') {
                  // 金额区间：传递数组 [最小值, 最大值]
                  if (typeof categoryTotalCondition.value === 'object' && categoryTotalCondition.value !== null) {
                    valueArr = [categoryTotalCondition.value.min, categoryTotalCondition.value.max]
                  }
    ...
    fillConditionData(conditionData) {
      ...
      conditionData.forEach(category => {
        ...
        if (category.total_condition && Array.isArray(category.total_condition) && category.total_condition.length > 0) {
          const categoryConfig = this.characteristicCategories.find(cat => cat.type === category.type)
            ...
            if (!this.totalConditions[category.type]) {
              const totalConditionsArray = categoryConfig.total_condition.map((tc, index) => {
                ...
                if (totalConditionData && totalConditionData.params) {
                  ...
                  if (tc.condition_type === 'timeRange') {
                    value = Array.isArray(params) ? params.map(timestamp => dayjs.unix(timestamp).valueOf()) : []
                  } else if (tc.condition_type === 'number_rang') {
                    value = Array.isArray(params) && params.length >= 2 
              ...
              if (subConfig) {
                ...
                if (subItem.params) {
                  ...
                  if (formItem.condition_type === 'timeRange') {
                    formItem.value = Array.isArray(params) ? params.map(timestamp => dayjs.unix(timestamp).valueOf()) : []
                  } else if (formItem.condition_type === 'number_rang') {
                    formItem.value = Array.isArray(params) && params.length >= 2
                      ? { min: params[0], max: params[1] }
                      : { min: 0, max: 0 }
                  } else if (formItem.condition_type === 'number_items') {
                    formItem.value = Array.isArray(params) ? params.map(id => ({ item_id: id })) : []
